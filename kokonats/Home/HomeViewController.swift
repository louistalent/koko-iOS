...
class HomeViewController: UIViewController {
    ...
    private func showTutorialView() {
        ...
        let isViewedTutorial = defaults.string(forKey: "viewOption")
        if isViewedTutorial == "isViewed"    {
           dismiss(animated: true)
        }
        else {
            let tutorview = tutorialViewController()
            self.present(tutorview, animated: true, completion: nil)
        }
    }
